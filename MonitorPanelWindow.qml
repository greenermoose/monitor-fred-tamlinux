...
// input without dismissing fred.monitor.
PanelWindow {
  id: root
  ...
  WlrLayershell.namespace: "fred-monitor-panel"
  ...
  readonly property real barStripSize: {
    ...
    return Math.max(bar.barSize, actual) + gap
  }
  ...
  // This is deliberately the only dismissal surface: it is bound to
  // `screen`, so clicks on any other output pass through to applications.
  MouseArea {
    id: dismissArea
    anchors.fill: parent
    enabled: root.open
    ...
    function inBarRegion(px, py) {
      if (root.barPos === "bottom") return py >= root.screenH - root.barStripSize
      if (root.barPos === "left") return px <= root.barStripSize
      if (root.barPos === "right") return px >= root.screenW - root.barStripSize
      return py <= root.barStripSize
    }
    ...
    function pressTargetAt(px, py) {
      ...
      for (var i = targets.length - 1; i >= 0; i--) {
        ...
        if (!target || !target.triggerPress || target.visible === false || target.opacity === 0) continue
        ...
        if (point.x >= position.x && point.x <= position.x + target.width
            && point.y >= position.y && point.y <= position.y + target.height) return target
      }
      return null
    }
